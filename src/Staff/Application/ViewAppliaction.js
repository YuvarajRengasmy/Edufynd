...
export const ViewApplication = () => {
  ...
  const makePayment = async () => {
    const stripe = await loadStripe(
      "pk_live_51OQ6F2A2rJSV7g6S1333dKPIqp5F7YahINaeS3w7fTFjiOcYneMtyXsE2QFiyGOkm9ruw6hNzZqiZSzUFGNdNVe10019LkXbRY"
    );
    ...
    const response = await fetch(
      "https://api.edufynd.in/api/payment/create-checkout-session",
      {
        method: "POST",
        headers: header,
                    ...
                    <div className="card border-0 rounded-1 shadow-sm p-3">
                      ...
                      <div className="d-flex justify-content-between align-items-center">
                          {status.map((item, index) => {
                            ...
                            const currentUser = tracks?.designation;
