...
isActive:"" 
  ...
  const handleSearch = (event) => {
    ...
    getNotificationSearch(data)
      .then((res) => {
        const universityList = res?.data?.result?.trainingList;
  ...
  const filterAgentList = (event) => {
    ...
    const data = {
        ...
        trainingTopic:inputs.trainingTopic,
        typeOfUser: inputs.typeOfUser,
        isActive: inputs.isActive
    };
    getFilterTraining(data)
    .then((res) => {
      const sortedStudents = res?.data?.result?.trainingList.sort((a, b) => {
  ...
  const pdfDownload = (event) => {
    ...
    getallTraining(notification)
      .then((res) => {
        ...
        templatePdf("trainingList", tablebody, "landscape");
      })
      .catch((err) => {
        console.log(err);
      });
      ...
      .then((res) => {
        ...
        result?.forEach((res) => {
          list.push({
            ...
            trainingTopic: res?.trainingTopic ?? "-",
  ...
  const deleteSelectedStudent = () => {
    if (selectedIds.length > 0) {
      Promise.all(selectedIds.map((id) =>deleteTraining(id)))
        .then((responses) => {
          toast.success("trainging deleted successfully!");
          setSelectedIds([]);
          setOpenDelete(false);
          getAllClientDetails();
        })
        .catch((err) => {
          console.log(err);
          toast.error("Failed to delete trainging.");
        });
    } else {
      toast.warning("No trainging selected.");
    }
  };
  const activateSelectedAgent = () => {
    if (selectedIds.length > 0) {
      // Send the selected IDs to the backend to activate the clients
      activeClient({ trainingIds: selectedIds })
        .then((response) => {
          console.log("Response:", response);
          toast.success("trainging activated successfully!");
          setSelectedIds([]); // Clear selected IDs after successful activation
          getAllClientDetails(); // Refresh the client list
        })
        .catch((err) => {
          console.error(err);
          toast.error("Already activate trainging.");
        });
    } else {
      toast.warning("No selected trainging.");
    }
  };
  ...
  const deactivateSelectedAgent= () => {
    if (selectedIds.length > 0) {
      ...
      deactivateClient({ trainingIds: selectedIds })
        .then((response) => {
          console.log("Response:", response);
          toast.success("trainging deactivated successfully!");
          setSelectedIds([]); // Clear selected IDs after successful deactivation
          getAllClientDetails(); // Refresh the client list
        })
        .catch((err) => {
          console.error(err);
          toast.error("Aready to deactivate trainging.");
        });
    } else {
      toast.warning("No selected trainging.");
    }
                    ...
                    <div>
                      ...
                      >
                        <div className="offcanvas-header">
                          <h5 id="offcanvasRightLabel">Filter Training</h5>
                          <button
                            type="button"
                          ...
                          <form>
                            <div className="from-group mb-3">
                              ...
                              <input
                                ...
                                placeholder="Search...trainingTopic"
                                style={{
                              ...
                              <input
                                ...
                                name="typeOfUser"
                                onChange={handleInputs}
                                placeholder="Search...Type Of User"
                                style={{
                                  fontFamily: "Plus Jakarta Sans",
                                  fontSize: "11px",
                                }}
                ...
                >
                  <div className="card-body">
                    ...
                    <p className="card-text">
                      Recently added training materials.
                    </p>
          ...
          <DialogContent>
            <div className="text-center p-4">
              <h5 className="mb-4" style={{ fontSize: "14px" }}>
                Are you sure you want to Delete <br /> the selected Training
                Material* ?
              </h5>
              <button
                type="button"
