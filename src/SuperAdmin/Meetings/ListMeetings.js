...
import {getallMeeting,deleteMeeting,getFilterMeeting, deactivateClient,activeClient  } from "../../api/Notification/meeting";
...
isActive:"" 
  ...
  const handleSearch = (event) => {
    ...
    getNotificationSearch(data)
      .then((res) => {
        const universityList = res?.data?.result?.meetingList;
  ...
  const filterAgentList = (event) => {
    ...
    getFilterMeeting(data)
    .then((res) => {
      const sortedStudents = res?.data?.result?.meetingList.sort((a, b) => {
  ...
  const pdfDownload = (event) => {
    ...
    getallMeeting(notification)
      .then((res) => {
        ...
        templatePdf("meetingList", tablebody, "landscape");
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  const deleteSelectedStudent = () => {
    if (selectedIds.length > 0) {
      Promise.all(selectedIds.map((id) =>deleteMeeting(id)))
        .then((responses) => {
  ...
  const activateSelectedAgent = () => {
    if (selectedIds.length > 0) {
      ...
      activeClient({ meetingIds: selectedIds })
        .then((response) => {
  ...
  const deactivateSelectedAgent= () => {
    if (selectedIds.length > 0) {
      ...
      deactivateClient({ meetingIds: selectedIds })
        .then((response) => {
          console.log("Response:", response);
