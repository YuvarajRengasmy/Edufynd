...
const AddSenderInvoice = () => {
 ...
 const id = new URLSearchParams(location.search).get("id");
  ...
  const [editApp, setEditApp] = useState(null);  // Store the application to be edited
...
const handleEditModule = (app) => {
  ...
  const handleSubmited = (event) => {
    ...
    const finalInvoiceData = {
      ...invoice,
      totalCourseFees: invoice.application.reduce((total, app) => total + (parseFloat(app.courseFeesAmount) || 0), 0),
      finalValue: invoice.application.reduce((total, app) => total + (parseFloat(app.courseFeesAmount) || 0), 0) * (invoice.commission || 0) / 100
    };
