...
export const ListGeneralEnquiry = () => {
  ...
  const handleSearch = (event) => {
    ...
    getCommonSearch(data)
      .then((res) => {
        const universityList = res?.data?.result?.generalEnquiryList;
  ...
  const pdfDownload = (event) => {
    ...
    getallGeneralEnquiry(student)
      .then((res) => {
        ...
        templatePdf("general Enquiry List", tablebody, "landscape");
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  const deleteSelectedstudent = () => {
    if (selectedIds.length > 0) {
      Promise.all(selectedIds.map((id) =>deleteGeneralEnquiry(id)))
        .then(() => {
  ...
  const activateSelectedStudent = () => {
    if (selectedIds.length > 0) {
      activeClient({ generalIds: selectedIds })
        .then(() => {
          toast.success('general(s) activated successfully!');
  ...
  const deactivateSelectedStudent = () => {
    if (selectedIds.length > 0) {
      deactivateClient({ generalIds: selectedIds })
        .then(() => {
          toast.success('general deactivated successfully!');
          setSelectedIds([]); // Clear selected IDs after success
          getAllStudentDetails(); // Refresh student list
        })
        .catch((err) => {
          console.error(err);
          toast.error('Failed to deactivate general(s).');
        });
    } else {
      toast.warning('No general selected.');
    }
                    ...
                    <div>
                      ...
                      >
                        <div className="offcanvas-header">
                          <h5 id="offcanvasRightLabel">Filter GeneralEnquiry</h5>
                          <button
                            type="button"
      ...
      <Dialog open={open}>
        <DialogContent>
          <div className="text-center m-4">
            ...
            >
              Are you sure you want to Delete <br /> the selected Studentt
              Enquiry ?
            </h5>
            <button
              type="button"
