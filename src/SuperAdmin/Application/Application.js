                        ...
                        <div className="position-relative my-4 mx-3">
                          <div className="progress" style={{ height: "8px" }}>
                            <div
                              ...
                              role="progressbar"
                              aria-label="Progress"
                              style={{ width: `${((currentStatusIndex + 1) / statuses.length) * 100}%`, backgroundColor: '#fe5722' }}
                          ...
                          {statuses.map((status, index) => (
                            <button
                              key={index}
                              type="button"
                              className={`position-absolute top-0 translate-middle btn btn-sm ${getStatusClassName(submittedStatus.has(index) ? selectedStatus : status.name)} rounded-pill`}
                              style={{ width: "2rem", height: "2rem", left: `${(index / (statuses.length - 1)) * 100}%` }}
                              disabled={index > currentStatusIndex || submittedStatus.has(index)}
                              onClick={() => handleStatusClick(index)}
                            >
                              <i className={`fa ${statusIcons[selectedStatus] || 'fa-' + statusIcons[status.name] || 'fa-circle'}`} aria-hidden="true" />
                            </button>
                          ))}
                          <div className='d-flex flex-row align-items-center justify-content-around'>
                            {statuses.map((status, index) => (
                              <div key={index} className='col-sm-2 fw-bold' style={{ fontSize: '12px' }}>
                                {status.name}
                              </div>
                            ))}
                          </div>
