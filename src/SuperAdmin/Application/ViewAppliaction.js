...
console.log("balaaaa", item)
  ...
  const handleTrackSubmit = (event) => {
    ...
    if (handleErrors(newErrorEducation)) {
        if (id) {
            const data = {
                _id: id, // Include the applicant's ID
                statusId: track.statusId, // Include the specific status ID to update
                newStatus: track.newStatus, // New status data
                progress: 100, // Assuming you want to set progress to 100%
                subCategory: selectedValues, // Set selected values for sub-category
                // Include any other required fields based on your backend
            };
